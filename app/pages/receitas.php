...
function vizualizar(id){
    ...
    $("#conteudoModal").load('pages/receitasDetalhe.php?id=' + id);
}
function novo(id){

    //antes de abrir a janela, preciso carregar os dados do cliente e preencher os campos dentro do modal
    $('#modalRecusar').modal('show');
    $("#conteudoModal").load('pages/receitasNovo.php');
}
  ...
  <div class="container-fluid container-fixed-lg sm-p-l-20 sm-p-r-20">
    <div class="inner">
      ...
      <ul class="breadcrumb">
        ...
        <li>Transparência</li>
        <li><a href="#" class="active">Receitas</a>
        </li>
      </ul>
      <!-- END BREADCRUMB -->
    </div>
...
<div class="col-md-9 col-sm-9 col-md-push-3 col-sm-push-3">
  ...
  $Atual=$pdo->prepare("SELECT * FROM receitas WHERE CdPrefeitura = '". $vAdmin->CdPrefeitura ."' AND Acao = 'Publicado' ORDER BY Ano DESC, Mes DESC");
  $Atual->execute();
...
if($ContaMes == 2){
  ...
  $sql="SELECT * FROM receitas WHERE CdPrefeitura = '". $vAdmin->CdPrefeitura ."' AND Acao = 'Publicado' AND Mes = '".$mesSeleciona."' AND Ano = '".$anoSeleciona."' ORDER BY Mes DESC, Ano DESC";
...
<div class="summary col-md-3 col-sm-3 col-md-pull-9 col-sm-pull-9">
  <?php
  $Caixa=$pdo->prepare("SELECT * FROM receitas WHERE CdPrefeitura = '".$vAdmin->CdPrefeitura."' AND Acao IN('Arquivo','Correcao','Cadastrando')");
  $Caixa->execute();
  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      ...
      <div class="panel-group" id="accordion1">
                              <?php
                              $Ano=$pdo->prepare("SELECT * FROM receitas WHERE CdPrefeitura = '". $vAdmin->CdPrefeitura ."' AND Acao = 'Publicado' GROUP BY Ano ORDER BY Ano DESC");
                              $Ano->execute();
                                ...
                                <div class="panel panel-default">
                                    ...
                                    <div id="collapse<?php echo $vAno->Ano;?>" class="panel-collapse collapse" style="height: 0px;">
                                        <div class="panel-body">
                                          <ul>
                                            <?php
                                            $Mes=$pdo->prepare("SELECT * FROM receitas WHERE CdPrefeitura = '". $vAdmin->CdPrefeitura ."' AND Acao = 'Publicado' AND Ano = '".$vAno->Ano."' GROUP BY Mes ORDER BY Mes ASC");
                                            $Mes->execute();
                                            ...
                                            foreach ($lMes as $vMes) {
                                              ...
                                              if($total == 2){
                                                ...
                                                ?>
                                                <li>
                                                  <a href="?p=receitas&m=<?php echo $mesSel;?>&a=<?php echo $vMes->Ano;?>"><?php echo retorna_mes_extenso($vMes->Mes);?></a>
                                                </li>
                                                <?php
                                            }
                                            ?>
