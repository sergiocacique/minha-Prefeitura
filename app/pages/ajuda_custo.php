...
<div class="col-md-9 col-sm-9 col-md-push-3 col-sm-push-3">
  ...
  $Atual=$pdo->prepare("SELECT * FROM ajuda_custo WHERE CdPrefeitura = '".$vAdmin->CdPrefeitura."' AND  Acao = 'Publicado' ORDER BY ano DESC, mes DESC");
  $Atual->execute();
...
if($ContaMes == 2){
  ...
  $sql="SELECT * FROM ajuda_custo WHERE CdPrefeitura = '".$vAdmin->CdPrefeitura."' AND  Acao = 'Publicado' AND mes = '".$mesSeleciona."' AND ano = '".$anoSeleciona."' ORDER BY mes DESC, ano DESC";
  ...
  <div class="panel panel-default">
    <div class="grid-title no-border">
        <h4>Ajuda de Custo de <strong><?php echo retorna_mes_extenso($mesSeleciona);?> <?php echo $anoSeleciona;?></strong></h4>
    </div>
  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      ...
      <div class="panel-group" id="accordion1">
                              <?php
                              $Ano=$pdo->prepare("SELECT * FROM ajuda_custo WHERE CdPrefeitura = '".$vAdmin->CdPrefeitura."' AND  Acao = 'Publicado' GROUP BY ano ORDER BY ano DESC");
                              $Ano->execute();
                                ...
                                <div class="panel panel-default">
                                    ...
                                    <div id="collapse<?php echo $vAno->ano;?>" class="panel-collapse collapse" style="height: 0px;">
                                        <div class="panel-body">
                                          <ul>
                                            <?php
                                            $Mes=$pdo->prepare("SELECT * FROM ajuda_custo WHERE CdPrefeitura = '".$vAdmin->CdPrefeitura."' AND Acao = 'Publicado' AND ano = '".$vAno->ano."' GROUP BY mes ORDER BY mes ASC");
                                            $Mes->execute();
                                            ...
                                            foreach ($lMes as $vMes) {
                                              ...
                                              if($total == 2){
                                                ...
                                                ?>
                                                <li>
                                                  <a href="?p=ajuda_custo&m=<?php echo $mesSel;?>&a=<?php echo $vMes->ano;?>"><?php echo retorna_mes_extenso($vMes->mes);?></a>
                                                </li>
                                                <?php
                                            }
                                            ?>
