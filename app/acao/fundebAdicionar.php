...
$Atual=$pdo->prepare("SELECT * FROM fundeb WHERE CdUsuario = '".$verTempo->CdUsuario."' AND DATE_FORMAT(DtCadastro, '%Y-%m') = '".$DtCadastro1."' ");
$Atual->execute();
...
$dir = '../../dinamico/municipio/'.$vAdmin->Pasta.'/anexo/'.$Ano.'/fundeb/';
...
if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $dir . $nome_final)) {
    ...
    echo "<h4>FUNDEB cadastrada com sucesso!</h4>";
    echo "</div>";

    $buscasegura=$pdo->prepare("INSERT INTO fundeb(CdPrefeitura,CdUsuario,Nome,Mes,Ano,Arquivo,Acao,DtCadastro,Protocolo)VALUES(:CdPrefeitura,:CdUsuario,:Nome,:Mes,:Ano,:Arquivo,:Acao,:DtCadastro,:Protocolo)");
...
<script language= "JavaScript">
    location.href="../index.php?p=fundeb"
</script>
