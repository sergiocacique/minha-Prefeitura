...
$Atual=$pdo->prepare("SELECT * FROM prestacao_conta WHERE Pasta = 'rgf' AND CdUsuario = '".$verTempo->CdUsuario."' AND DATE_FORMAT(DtCadastro, '%Y-%m') = '".$DtCadastro1."' ");
$Atual->execute();
...
$dir = '../../dinamico/municipio/'.$vAdmin->Pasta.'/'.$Ano.'/rgf/';
...
if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $dir . $nome_final)) {
    ...
    echo "<h4>Relatório de Gestão cadastrada com sucesso!</h4>";
    echo "</div>";
